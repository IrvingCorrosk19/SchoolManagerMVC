...
namespace SchoolManager.Controllers
{
    public class StudentAssignmentController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateAssignments(Guid userId, List<Guid> groupIds, List<Guid> gradeLevelIds)
        {
            var user = await _userService.GetByIdWithRelationsAsync(userId);
            if (user == null) return NotFound();

            await _userService.UpdateAsync(user, new List<Guid>(), groupIds, gradeLevelIds);
